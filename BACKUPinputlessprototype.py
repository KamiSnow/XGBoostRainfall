...
def train_and_save_models():
    ...
    feature_importance_df = pd.Series(reg_final.feature_importances_, index=FEATURES).sort_values(ascending=False)
    print(feature_importance_df)
    ...
    if not np.isnan(mape):
        print(f"MAPE: {mape:.2f}% (calculated on {valid_mape_mask.sum()} samples)")
    ...
    print("="*60)
    print(f"Global metrics stored: {GLOBAL_METRICS}")
